...
import Login from "../components/generatePage/login";
import Register from "../components/generatePage/register";
...
export default function Generate() {
  ...
  return (
    <section className="flex min-w-screen min-h-screen bg-[#292929] items-center justify-center">
      {auth.currentUser
        ? (loaded
          ? (
        ...
        : (
          <div className="max-w-screen max-h-screen">
            {!register ? <Login register={() => setRegister(true)}></Login> : <Register signIn={() => setRegister(false)}></Register>}
          </div>
        )}
    </section>
  );
